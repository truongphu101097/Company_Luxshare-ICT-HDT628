...
namespace MerryKing
{
  static  class play
    {
       static int   _stream = 0;
        #region 拿到播放或者录音装置的索引值
        private static Int32 GetIndexOfAudioCard(Int32 type, String DeviceName)
        {
        ...
        #endregion
        #region 播放音乐
        private static bool playmusic( String DeviceName,IntPtr handle,string music,bool restart)
        {
        ...
        #endregion
        #region 对外方法 播放
        public static bool playTest( String DeviceName,string music, bool restart)
        {
            if (!playmusic( DeviceName, IntPtr.Zero, Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location) + $@"\{music}", restart)) return false;
           // bool flag = messagebox(message, owner);
           // StopPlay();
            return true;
        }
        #endregion
        #region  消息提示
        private static bool messagebox(string message, IWin32Window owner)
        {
            if (MessageBox.Show(owner, message, "提示", MessageBoxButtons.YesNo) == DialogResult.No) return false;
            return true;
        }
        #endregion
        #region 录音
        static SoundRecord recorder = new SoundRecord();
        private static void Record(Int32 type, String DeviceName)
        {
            int  PlayerIndex = GetIndexOfAudioCard(type, DeviceName);
            string wavfile = null;
            wavfile = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location)+@"\Record.wav";
            recorder.InitCaptureDevice(PlayerIndex);
            recorder.SetFileName(wavfile);
            recorder.RecStart();
            //MessageBox.Show("停止录音?");
            //Thread.Sleep(1000);
            //recorder.RecStop();//录音结束   
        }
        #endregion
        #region 录音 对外方法
        public static bool RecordTest( String DeviceName)
        {
            try
            {
                Record(1, DeviceName);
    ...
    class SoundRecord
    {
        ...
        Microsoft.DirectX.DirectSound.Capture mCapDev = null;          //音频捕捉设备
        ...
        public bool InitCaptureDevice(int SoundRecordIndex)
        {
            ...
            if (flag)
            {
                ...
                try
                {
                    this.mCapDev = new Microsoft.DirectX.DirectSound.Capture(guidDev);
                }
