    ...
    public class OldButtonTest
    {
        ...
        private void ReadCompleted(IAsyncResult iResult)
        {
            ...
            try
            {
                ...
                {
                    if (Convert.ToString(arrBuff[flag[i]], 16).PadLeft(2, '0') != readdata[i]) { testflag = false; break; }
                }
                if (testflag)
                {
        ...
        /// <returns></returns>
        public bool Buttontest(string readdata, string indexs, string name, string handle, int length = 1000)
        {
            _length = length;
